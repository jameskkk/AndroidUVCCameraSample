...
void Java_com_k2jstudio_uvccam_NativeUVCcam_loadNextFrame(JNIEnv* env, jobject thiz, jobject bitmap) {
    ...
    if(!RGB_BUFFER || !Y_BUFFER) {
    	LOGD("Unable to load frame, buffers not initialized");
        return;
    }
...
void Java_com_k2jstudio_uvccam_NativeUVCcam_stopCamera(JNIEnv* env, jobject thiz) {
    stop_camera(&DEVICE_DESCRIPTOR, RGB_BUFFER, Y_BUFFER);
}

jboolean Java_com_k2jstudio_uvccam_NativeUVCcam_cameraAttached(JNIEnv* env, jobject thiz) {
